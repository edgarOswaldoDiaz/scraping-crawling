...
def process_directory(directory):
    ...
    # Eliminar archivos no útiles, innecesarios y duplicados
    for file_list, reason in [
        (archivos_no_utiles, "no útil"),
        (archivos_innecesarios, "innecesario"),
        (archivos_duplicados, "duplicado"),
    ]:
        for file_path in file_list:
            try:
                os.remove(file_path)
                print(f"'{file_path}' eliminado por ser {reason}.")
            except Exception as e:
                print(f"Error al intentar eliminar '{file_path}': {e}")

# Llamar a la función
process_directory('C:/Users/Sam/Downloads/INEGI_muestra_scraping/muestra/omgmexico.com.mx')
